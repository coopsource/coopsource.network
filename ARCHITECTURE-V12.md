...
**Updated:** 2026-07-04. Grounded in `docs/plans/2026-07-04-atproto-shared-spaces-research.md` (upstream ecosystem state) and a full code audit of `main` at merge `5c0f8b0`.
...
| Commit digest | ECMH | **LtHash** (lattice-based, quantum-secure) | `EcmhVerifier` → rename to `CommitDigestVerifier`; port shape unchanged |
...
**Placement rule:** push a feature down a layer unless doing so dilutes its generality. Test: *would Roomy (or another non-cooperative group app) use this unmodified?* If yes, it belongs in Layer 3 or lower.

- **Layer 1 — Spaces** (Holmgren / Bluesky). A space is `(authority: DID, type: NSID, skey: string)` with one member list of `(DID, read|write)` tuples, each member holding a per-space permissioned repo on their own PDS. Reads gated by the list; **writes enforced by readers** at the application layer. Sync is pull-based (oplog `listRepoOps` + CAR fallback), digest is LtHash. *"space authority" ≙ our `arbiterDid`.*
...
**The OAuth-spaces seam** (Axis 1 ↔ Axis 2) now has a concrete shape (Diary 6): **delegation token** (minted by the user's PDS) → **client attestation** (signed by the app) → **space credential** (issued by the space authority), plus per-space client allow/deny lists. The three distinct failure modes map to it directly: *scope not granted* (Axis 1) / *not in space* (Axis 2) / *app not authorized for this space* (client-policy). Code must keep them distinct.

*Reference implementation of the gate:* see `apps/api/src/routes/federation.ts` `/membership/approve` — Axis 2 authority check (caller must be the coop DID or an active owner/admin) runs before any mutation and returns 403 naming `axis: 'spaces'`.
...
The Arbiter's **identity-vs-authorization split** aligns with our axes: its membership layer answers Axis 2; its uploadable Rego policy is the arbiter's *server-side* authorization, distinct from CSN's Axis-3 `GovernancePluginSet` which sits **above** the arbiter boundary. Do not push cooperative governance logic down into arbiter policy.

`SpaceRef = { arbiterDid: DID, spaceKey: string, expectedSpaceType?: NSID }` — URI-scheme-independent. Space types are string constants in `packages/arbiter-client/src/space-ref.ts` (`membersSpace`, `roleSpace`, `MEMBERS_SPACE_TYPE`, …), **not lexicons**.
...
- **Digest:** LtHash, behind the `CommitDigestVerifier` port (fail-closed default; full-resync on mismatch). No real verifier ships yet — `FailClosedCommitDigestVerifier` is the default; the `UnsafeAlwaysOk*` variant is test-only and throws in production.
- **Sync:** pull-based. The spaces consumer subscribes to arbiter write-notifications, pulls changed records from member PDSes, cross-checks each record's `authorDid` against the resolved member list, verifies the digest, and projects into Postgres. **Records from non-members are discarded; resolution that is partial or stale fails closed.** This reader-side enforcement is not a stopgap — it is *the* write-enforcement mechanism per Diary 6.
- **Trust anchor:** pulled records are *claimed* until cross-checked against the authoritative member list.
...
| Layer | Package | State |
|---|---|---|
| 1 Spaces | `packages/spaces-consumer` | Real skeleton, **flag-gated off**. `SpaceRef`, `SpacesConsumer`, `GroupDirectoryPort`, `SpaceCredentialStore`, Kysely cursor store, tests. Digest verifier is fail-closed sketch; notification/pull/repo ports are in-memory sketches. |
...
| 2 | none | Drift alignment: `CommitDigestVerifier` rename, URI helpers, dep bumps, port↔lexicon JSDoc, review cleanup |
| 3 | soft | Arbiter convergence + membership reads through the port + review carry-ins (suspension, lifecycle events, consent-overwrite, roster-`partial`, DID rotation) |
...
| Spaces primitives (extend vs **`@atproto/space`** from PR #5187) | Use when published | each sweep |
...
1. Full credential-seam mechanics — known shape (three tokens), unshipped; firm up in Phase 4 against `@atproto/space`.
...
3. Don't bake the URI scheme or digest algorithm as constants — helpers/ports only. (`at://…/space/…` and LtHash are current values, still behind helpers.)
4. HappyView is a reference/harness — the build-vs-use decision (§10) defaults to *build* our own AppView; re-evaluate at Phase 5, don't migrate onto its Lua/WASM model by default.
