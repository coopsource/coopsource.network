...
This substrate is restricted to development and tests; the API refuses to
start in production without real PDS and PLC URLs.
...
The local simulation is a development/test adapter. In production, the API
requires `PDS_URL` or `COOP_PDS_URL`, requires a URL-valued `PLC_URL`, and
rejects the default PDS admin credential.

| Variable | Default | Description |
|----------|---------|-------------|
| `PLC_URL` | `local` | `local` = DB-backed `LocalPlcClient` in development/test; production requires a real PLC directory URL (e.g. `https://plc.directory` or `http://localhost:2582` for dev containers) |
| `PDS_URL` | (none) | When set, activates `AtprotoPdsService`. A PDS URL is required in production unless `COOP_PDS_URL` is set |
| `PDS_ADMIN_PASSWORD` | `admin` | PDS admin password for HTTP Basic auth; production rejects the development default |
...
| `PDS_URL` | Yes | Internal API-to-PDS URL (`http://pds:3000` in the provided Compose stack) |
| `PLC_URL` | Yes | Internal API-to-PLC URL (`http://plc:2582` in the provided Compose stack) |
...
Requires `infrastructure/.env` with all required variables set, including a
real `PLC_URL`, `PDS_URL`, and `PDS_ADMIN_PASSWORD`. The production stack does
not fall back to `LocalPdsService` or `LocalPlcClient`.
