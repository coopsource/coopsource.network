...
<div class="mx-auto max-w-3xl space-y-6">
  ...
  {:else}
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
      <!-- Alignment Zones -->
      <div>
        <h2 class="mb-3 text-sm font-medium text-green-700">
          Alignment Zones ({data.map.alignmentZones.length})
        </h2>
            ...
            {#each data.map.alignmentZones as zone}
              <div class="rounded-lg border border-green-200 bg-green-50 p-3">
                ...
                <p class="mt-1 text-xs text-green-600">
                  {zone.participants.length} participant{zone.participants.length !== 1 ? 's' : ''}
                </p>
                {#if zone.interestsInvolved.length > 0}
                  <div class="mt-2 flex flex-wrap gap-1">
                    {#each zone.interestsInvolved.slice(0, 3) as interest}
                      <span class="rounded bg-green-100 px-1.5 py-0.5 text-xs text-green-700 line-clamp-1">
                        {interest.slice(0, 60)}{interest.length > 60 ? '…' : ''}
                      </span>
                    {/each}
                  </div>
                {/if}
      ...
      <!-- Conflict Zones -->
      <div>
        <h2 class="mb-3 text-sm font-medium text-red-700">
          Conflict Zones ({data.map.conflictZones.length})
        </h2>
                ...
                <div class="flex items-start justify-between">
                  ...
                  <span class="rounded-full px-2 py-0.5 text-xs font-medium
                    {zone.severity === 'high' ? 'bg-red-200 text-red-800' :
                     zone.severity === 'medium' ? 'bg-yellow-200 text-yellow-800' :
                     'bg-gray-200 text-gray-800'}">
                    {zone.severity}
                  </span>
                </div>
                <p class="mt-1 text-xs text-red-600">
                  {zone.stakeholders.length} stakeholder{zone.stakeholders.length !== 1 ? 's' : ''} involved
                </p>
    ...
    {#if data.map.aiAnalysis}
      <div class="rounded-lg border border-purple-200 bg-purple-50 p-4">
        ...
        <pre class="mt-2 whitespace-pre-wrap text-sm text-purple-700">
          {JSON.stringify(data.map.aiAnalysis, null, 2)}
        </pre>
      </div>
    {/if}
