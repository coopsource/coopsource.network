...
<div class="space-y-4">
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      <a href="{$workspacePrefix}/agreements" class="text-sm text-[var(--cs-text-muted)] hover:text-[var(--cs-text)]">&larr; Agreements</a>
      <h1 class="text-xl font-semibold text-[var(--cs-text)]">Templates</h1>
    </div>
    <a
      href="{$workspacePrefix}/agreements/templates/new"
  ...
  {:else}
    <div class="space-y-2">
      {#each data.templates as template}
        <a
          href="{$workspacePrefix}/agreements/templates/{template.id}"
