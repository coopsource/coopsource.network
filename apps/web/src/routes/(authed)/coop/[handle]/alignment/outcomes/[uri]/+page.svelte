...
<div class="mx-auto max-w-2xl space-y-6">
  <div class="flex items-start justify-between">
    ...
    <span class="rounded-full px-2.5 py-1 text-xs font-medium {categoryBadgeClass(o.category)}">{o.category}</span>
  </div>
  ...
  {#if nextStatuses[o.status]?.length}
    <div class="flex items-center gap-2">
      ...
      {#each nextStatuses[o.status] as target}
        <form method="POST" action="?/updateStatus" use:enhance={() => { updatingStatus = true; return async ({ update }) => { updatingStatus = false; await update(); }; }} class="inline">
          <input type="hidden" name="status" value={target} />
          <button type="submit" disabled={updatingStatus} class="rounded-md px-2.5 py-1 text-xs font-medium disabled:opacity-50 {statusButtonClass[target] ?? 'bg-gray-200 text-gray-700'}">{target}</button>
        </form>
      {/each}
    </div>
  {/if}
  ...
  <div class="rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg-card)] p-4">
    ...
    <div class="mt-4 border-t border-[var(--cs-border)] pt-4">
      ...
      {/if}
      <form method="POST" action="?/support" use:enhance={() => { supporting = true; return async ({ update }) => { supporting = false; await update(); }; }} class="mt-2 space-y-3">
        <div>
          <label for="level" class="block text-xs text-[var(--cs-text-secondary)]">Support Level</label>
          <select id="level" name="level" class="mt-0.5 block w-full rounded border border-[var(--cs-border)] px-2 py-1 text-sm">
        ...
        <div>
          ...
          <textarea id="conditions" name="conditions" rows="2" maxlength="2000" class="mt-0.5 block w-full rounded border border-[var(--cs-border)] px-2 py-1 text-sm"></textarea>
        </div>
        <button type="submit" disabled={supporting} class="rounded-md bg-[var(--cs-primary)] px-3 py-1.5 text-xs font-medium text-[var(--cs-text-on-primary)] hover:bg-[var(--cs-primary-hover)] disabled:opacity-50">
