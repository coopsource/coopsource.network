...
<div class="mx-auto max-w-lg space-y-6">
  ...
  <form method="POST" use:enhance={() => { submitting = true; return async ({ update }) => { submitting = false; await update(); }; }} class="space-y-4">
    <div>
      ...
      <input id="title" name="title" type="text" required maxlength="255" class="mt-1 block w-full rounded-md border border-[var(--cs-border)] px-3 py-2 text-sm focus:border-[var(--cs-border-focus)] focus:outline-none focus:ring-1 focus:ring-[var(--cs-ring)]" />
    </div>
    <div>
      <label for="description" class="block text-sm font-medium text-[var(--cs-text-secondary)]">Description</label>
      <textarea id="description" name="description" rows="3" maxlength="3000" class="mt-1 block w-full rounded-md border border-[var(--cs-border)] px-3 py-2 text-sm focus:border-[var(--cs-border-focus)] focus:outline-none focus:ring-1 focus:ring-[var(--cs-ring)]"></textarea>
    </div>
    <div>
      <label for="category" class="block text-sm font-medium text-[var(--cs-text-secondary)]">Category</label>
      <select id="category" name="category" class="mt-1 block w-full rounded-md border border-[var(--cs-border)] px-3 py-2 text-sm">
        ...
        <div class="rounded-md border border-[var(--cs-border)] p-3 space-y-2">
          <div class="grid grid-cols-2 gap-2">
            <div>
              ...
              <input id="criteria_metric_{i}" name="criteria_metric_{i}" type="text" maxlength="500" class="mt-0.5 block w-full rounded border border-[var(--cs-border)] px-2 py-1 text-sm" />
            </div>
            <div>
              <label for="criteria_target_{i}" class="block text-xs text-[var(--cs-text-secondary)]">Target</label>
              <input id="criteria_target_{i}" name="criteria_target_{i}" type="text" maxlength="500" class="mt-0.5 block w-full rounded border border-[var(--cs-border)] px-2 py-1 text-sm" />
            </div>
          </div>
          <div>
            <label for="criteria_timeline_{i}" class="block text-xs text-[var(--cs-text-secondary)]">Timeline (optional)</label>
            <input id="criteria_timeline_{i}" name="criteria_timeline_{i}" type="text" maxlength="200" class="mt-0.5 block w-full rounded border border-[var(--cs-border)] px-2 py-1 text-sm" />
          </div>
        </div>
      {/each}
      <button type="button" onclick={() => criteriaCount++} class="text-xs text-[var(--cs-primary)] hover:text-[var(--cs-primary-hover)]">+ Add Criterion</button>
    </fieldset>
