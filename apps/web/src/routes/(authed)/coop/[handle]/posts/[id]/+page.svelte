...
<div class="mx-auto max-w-3xl space-y-6">
  ...
  <div class="rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg-card)] p-6">
    <div class="mb-4 flex items-start justify-between gap-4">
      <h1 class="text-xl font-semibold text-[var(--cs-text)]">{thread.title ?? 'Untitled thread'}</h1>
  ...
  <div class="rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg-card)]">
    <h2 class="border-b border-[var(--cs-border)] px-5 py-4 text-sm font-semibold text-[var(--cs-text)]">Posts ({posts.length})</h2>
    {#if posts.length === 0}
            ...
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-xs text-[var(--cs-text-muted)]">{new Date(post.createdAt).toLocaleString()}</span>
                  {#if post.editedAt}
  ...
  <div class="rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg-card)] p-5">
    ...
    <form method="POST" action="?/createPost" use:enhance={() => { submitting = true; return async ({ update }) => { submitting = false; postBody = ''; await update(); }; }} class="space-y-4">
      <textarea name="body" bind:value={postBody} required rows={4} class="block w-full rounded-md border border-[var(--cs-input-border)] bg-[var(--cs-input-bg)] px-3 py-2 text-sm text-[var(--cs-text)] focus:border-[var(--cs-border-focus)] focus:outline-none focus:ring-1 focus:ring-[var(--cs-ring)]" placeholder="Write your post..."></textarea>
