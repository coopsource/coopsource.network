...
<div class="mx-auto max-w-3xl space-y-6">
    ...
    <div class="mt-4">
      {#if isMember}
        <form method="POST" action="?/leave" use:enhance>
          <button type="submit" class="rounded-md border border-red-300 px-3 py-1.5 text-sm font-medium text-red-700 hover:bg-red-50">Leave network</button>
        </form>
      {:else}
        <form method="POST" action="?/join" use:enhance>
          <button type="submit" class="rounded-md bg-[var(--cs-primary)] px-3 py-1.5 text-sm font-medium text-[var(--cs-text-on-primary)] hover:bg-[var(--cs-primary-hover)]">Join network</button>
        </form>
      {/if}
    </div>
  </div>
  ...
  <div>
    ...
    {#if members.length === 0}
      <EmptyState title="No members yet" description="This network has no cooperative members." />
    {:else}
      <div class="space-y-2">
          ...
          <div class="rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg-card)] p-4">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 flex-1">
                ...
                {/if}
                {#if member.joinedAt}
                  <p class="mt-1 text-xs text-[var(--cs-text-muted)]">Joined {new Date(member.joinedAt).toLocaleDateString()}</p>
                {/if}
              </div>
