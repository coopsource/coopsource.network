      ...
      <div class="mt-4 space-y-3">
        {#each data.availableServices as service}
          ...
          <div class="flex items-center justify-between rounded-md border border-[var(--cs-border)] p-3">
            ...
            {:else}
              <form method="POST" action="?/connect" use:enhance={() => { submitting = true; return async ({ result, update }) => { submitting = false; if (result.type === 'success' && result.data?.redirect) { window.location.href = result.data.redirect as string; } else { await update(); } }; }}>
                <input type="hidden" name="service" value={service} />
                <button type="submit" disabled={submitting} class="rounded-md bg-[var(--cs-primary)] px-3 py-1.5 text-sm font-medium text-[var(--cs-text-on-primary)] hover:bg-[var(--cs-primary-hover)] disabled:opacity-50">Connect</button>
              </form>
            {/if}
          </div>
        {/each}
