...
**Checklist (updated 2026-07-30):** `privateRecordService` + `private-record-service.ts` + `private_record` table + `routes/private/records.ts`; `governanceLabeler` (and its `ProposalService` injection); `http/signing.ts` (RFC 9421); `cooperative-link-service.ts` + `cooperative_link` table; local PDS/PLC classes (`LocalPdsService`, `LocalPlcClient`, `LocalBlobStore`, local `firehose.ts`, `cooperative-provisioning.ts`). The legacy `VisibilityRouter` was removed in Phase 4 and replaced by the named `GovernanceRecordPlacementPort`; the default adapter still reads the existing CSN visibility policy. The dormant outbound federation client and signing-key resolver were removed in the first Phase 6 checkpoint. The federation outbox table and runtime had already been removed in V7; the second checkpoint removed its final current-code comments. Each remaining removal: delete code + container wiring + schema table + tests in one commit per subsystem; `pnpm build && pnpm test` between commits.
