...
`GovernancePluginSet.quorum`. Do not move additional service logic until the
adapter layer is tested against current API suites.
...
1. Add `DelegateChainsPlugin` only after reviewing
